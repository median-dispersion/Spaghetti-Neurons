...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, required=False, help="File path to the output JSON file")

    arguments = parser.parse_args()

    arguments.input = Path(arguments.input)

    if arguments.output is None: arguments.output = f"{arguments.input.stem}.json"
    ...
    with open(arguments.input, "rb") as file:
        ...
        network["layers"].append({
            "neurons": [{"bias": None, "connections": []}] * inputs
        })
